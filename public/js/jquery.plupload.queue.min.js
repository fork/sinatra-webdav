...
(function($) {
	...
	function renderUI(id, target) {
		...
		*/
		target.prepend(
			'<div class="plupload_shadow"></div>' +
			'<div class="plupload_filelist_toggle" style="display:none;">File upload<div class="icon"></div></div>' +
			'<div class="plupload_wrapper plupload_scroll">' +
				'<div id="' + id + '_container" class="plupload_container">' +
					'<div class="plupload">' +
						...
						'<div class="plupload_content">' +
							'<div class="plupload_filelist_header">' +
								...
								'<div class="plupload_file_status"><span>' + _('Status') + '</span></div>' +
								'<div class="plupload_file_size">' + _('Size') + '</div>' +
								'<span class="plupload_clearer">&nbsp;</span>' +
							'</div>' +
			...
			this.each(function() {
				...
				*/
				uploader = new plupload.Uploader($.extend({
					dragdrop : true,
					container : id,
					webdavContainer: target.parents('form.column')
				}, settings));
				// webdav costomization end
				...
				function updateList() {
					...
					$.each(uploader.files, function(i, file) {
						...
						*/
						$('#' + file.id + '.plupload_delete a,#' + file.id + '.plupload_failed a,#' + file.id + '.plupload_done a').click(function(e) {
							$('#' + file.id).remove();
							uploader.removeFile(file);
				...
				uploader.bind('StateChanged', function() {
					...
					// webdav costomization start
					} else if (uploader.state == plupload.DONE || uploader.state == plupload.FAILED || uploader.state == plupload.STOPPED) {
						$('div.plupload_buttons', target).show();
						column = target.parents('.column');
						path = $.cookie('webdav.' + column.attr('id') + '.href')
						Controller('directory').apply(column, [path]);
					// webdav costomization end
					} else {
